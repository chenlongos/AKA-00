-- block.lua — 追方块并抓起来（demo "block" 专用）
--
-- **一个 demo 一个脚本**：脚本名 = demo 名 = 模型名，三样东西按同一个名字对齐
-- （模型 demo/models/block.cvimodel、本脚本、参数 demo/configs/block.json）。
-- 好处是每个 demo 的判据可以各自调，改这里不影响别的 demo —— 想给方块单开一套
-- 就动 tennis.lua，两边互不干扰。
--
-- 用法:
--   POST /api/demo/init {"name":"block"}     ← 界面上的"开始"走这条
--   POST /api/script/run
--   {"script":"block", "max_seconds":30, "params":{"target_size":300, "speed":20}}
--
-- 参数（从 params() 里读，界面 Demo 页每个卡片可调、存在 demo/configs/block.json）:
