    ...
    while True:
        ...
        try:
            ...
            if result and len(result) > 0:
                ...
                center_y = y + h // 2
                if center_x < left :
                    # car_controller.set_direction("left")
                    print(" 目标在左")
                    turn_left(left_motor, right_motor, 50)
                elif center_x > right :
                    # car_controller.set_direction("right")
                    print(" 目标在右")
                    turn_right(left_motor, right_motor, 50)
                elif center_y < top :
                    # car_controller.set_direction("up")
                    print(" 目标在上")
                    backward(left_motor, right_motor, 50)
                elif center_y > bottom :
                    # car_controller.set_direction("down")
                    print(" 目标在下")
                    forward(left_motor, right_motor, 50)
                else:
                    # car_controller.set_direction(None)
                    print(" 目标在中心")
                    if current_status == RobotStatus.FIND_TENNIS:
                        # grab(servo)
